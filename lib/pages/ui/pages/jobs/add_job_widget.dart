...
class AddJobWidget extends StatefulWidget {
  const AddJobWidget({this.jobs, this.isEditing = true, super.key});
...
//fetch vehicles
    ...
    _truckModule
        .fetchTrucksReges(userModule.currentUser.value.tenantId!)
        .then((value) {
      setState(() {
        _trucks = value;
        if (widget.isEditing) {
  ...
  void updateJob() async {
    ...
    var res = await _jobModule.updateJob((widget.jobs?.id ?? ''), {
      'vehicleId': _truck,
      'createdBy': userModule.currentUser.value.id,
