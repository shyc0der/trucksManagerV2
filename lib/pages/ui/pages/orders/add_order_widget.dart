...
class _AddOrderWidgetState extends State<AddOrderWidget> {
 ...
 final JobTypeModule jobTypeModule=Get.put(JobTypeModule());
...
final  Map<String, dynamic> _driver = {'None': 'None'};
  ...
  void initState() {
    ...
    //fetch jobTitles
        ...
        jobTypeModule.fetchJobTypesAsString(userModule.currentUser.value.tenantId!)
        .then((value) {
      setState(() {
        if (value.isNotEmpty) {
          _jobTitles = value;
    ...
    //fetch drivers email
    _userModule.fetchUsersName(userModule.currentUser.value.tenantId!).then((value) {
      setState(() {
