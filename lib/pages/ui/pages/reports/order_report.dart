  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: StreamBuilder<List<OrderModel>>(
          stream: _orderModules.fetchAllWhereOrderByDate(_dateTimeRange, userModule.currentUser.value.tenantId!),
          builder: (context, snapshot) {
            var data = snapshot.data ?? [];
            var totalAmount = data.fold<double>(
                0.0, (amount, order) => amount + (order.amount ?? 0.0));
            var newMap =
                groupBy(data, (OrderModel orderModel) => orderModel.title);
