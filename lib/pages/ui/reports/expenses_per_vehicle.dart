...
class ExpensesPerVehicleReport extends ReportGeneratorModule {
  ExpensesPerVehicleReport(this.vehicleId,
      {this.expenseType = const [], required this.dateRange});
  ...
  Future<String> get _vehicleReg async =>
      (await _truckModule.fetchTruckById(vehicleId)).vehicleRegNo ?? 'null';
  ...
  Future getExpenses() async {
    expenses = await _expenseModule
        .fetchByTruckExpenses(
            vehicleId, userModule.currentUser.value, userModule.currentUser.value.tenantId!)
        .first;
...
class ExpensesPerVehicleReportController extends GetxController {
  ...
  ReportGeneratorModule filter() {
    return ExpensesPerVehicleReport(selectedTruck.value,
        expenseType: selectedExpenseTypes, dateRange: dateRange.value);
  }
}
