...
class JobsReport extends ReportGeneratorModule {
 ...
 final UserModule userModule = Get.find<UserModule>();
  ...
  Future getJobs() async {
    var _jobs = await _jobModule2
        .fetchAllWhereDateRangeJobs(dateRange, userModule.currentUser.value.tenantId!)
        .first;
    if (driverId != null) {
    ...
    for (var _job in _jobs) {
      ...
      if (selectedJobTypes.isNotEmpty &&
          !selectedJobTypes.contains(order.title)) {
        continue;
      }
      ...
      // get customer
      final customer =
          await userModule.fetchCustomerById(order.customerId ?? 'null');

      String customerName =
          "${customer.firstName ?? ''}  ${customer.lastName ?? ''}";
      // get driver name
      final driver = await userModule.getUserById(_job.driverId ?? 'null');
