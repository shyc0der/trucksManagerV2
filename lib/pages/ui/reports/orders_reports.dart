...
import 'package:truck_manager/pages/ui/reports/reports_generator_module.dart';
...
class OrderReport extends ReportGeneratorModule {
  ...
  Future getOrder() async {
    var _orders =
        await _orderModule.fetchAllWhereOrderByDate(dateRange, userModule.currentUser.value.tenantId!).first;
    // filter customer
    if (customerId != null) {
    ...
    for (var _order in _orders) {
      ...
      // get job by Orderid
      final _job =
          await _jobModule2.fetchJobsByOrderId(_order.id ?? 'null', userModule.currentUser.value.tenantId!);
      double _expensesPerJobTotal = 0;

      if (_job != null) {
        // then get expenses by job id
        final _exs = await _expenseModule
            .fetchByJobExpenses(
                _job.id ?? 'null', userModule.currentUser.value.tenantId!, userModule.currentUser.value)
            .first;
      ...
      // get customer name
      final _customer =
          await _customerModule.fetchCustomerById(_order.customerId ?? 'null');
      String _customerName =
          "${_customer.firstName ?? ''}  ${_customer.lastName ?? ''}";

      orders.add(OrdersReoprtModel(
          expensesAmount: _expensesPerJobTotal,
          orderModel: _order,
